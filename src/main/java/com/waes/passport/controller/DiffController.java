...
import com.waes.passport.model.DiffModel;
...
public class DiffController {
	 ...
	 * @param diffModel  payload with the base64 value
	...
	public ResponseEntity<?> left(@PathVariable("id") String id, @Valid @RequestBody DiffModel diffModel) throws NumberFormatException, UnsupportedEncodingException {
		...
		diffService.save(Long.parseLong(id), EncodingUtil.decodeBase64ToByteArray(diffModel.getEncodedData()), null);
	...
	public ResponseEntity<?> right(@PathVariable("id") String id, @Valid @RequestBody DiffModel diffModel) throws NumberFormatException, UnsupportedEncodingException {
		...
		diffService.save(Long.parseLong(id), null, EncodingUtil.decodeBase64ToByteArray(diffModel.getEncodedData()));
        return new ResponseEntity<ResponseModel>(new ResponseModel(Constants.REQUEST_RECEIVED_SUCCESSFULLY), HttpStatus.CREATED); 
	}
